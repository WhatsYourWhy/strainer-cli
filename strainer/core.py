"""Core summarization, tagging, and text-cleaning logic."""
...
def summarize(
    ...
    *,
    ...
    Returns a plain string unless *include_anchors* is ``True``, in which case
    a dict with ``"text"`` and ``"anchors"`` keys is returned.
    """
    ...
    if MODEL:  # semantic mode
        ...
        scores = [
            _cosine_similarity(doc_vec, sentence_vec)
            for sentence_vec in normalized_embeddings[1:]
        ]
...
def tag(
    ...
    *,
    ...
    """Return the *top* most-frequent topic keywords from *text*."""
    words = re.findall(r"\w+", text.lower())
    stop = {
        "the", "and", "for", "with", "this", "that", "from", "were", "been",
        "have", "using", "used", "which", "their", "they", "will", "would",
        "there", "these", "about", "when", "what", "where", "is", "are",
        "was", "not", "but", "all", "into", "can", "has", "more", "one",
        "its", "out", "also", "than", "other", "some", "very", "only",
        "time", "just", "even", "most", "like", "may", "such", "each",
        "new", "based", "our", "results", "study", "method", "approach",
        "proposed",
    }
